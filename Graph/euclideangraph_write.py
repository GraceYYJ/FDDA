...
def euclideanDist(features1, features2):
    """Calculate the euclidean distance between two 48 bit features"""
...
#nodes
    ...
    # hyfile_node.create_dataset("featuresnode"+num,data=features)
...
#relations
    ...
    hyfile_relation = h5py.File(os.path.join(__PATH2__, 'euclideanrelation'+num+'_'+str(div)+'.hy'), 'w')
    ...
    hyfile_relation.create_dataset("euclideanrelation"+num+"_"+str(div),data=relations)
    precisions=np.asarray(precisions,dtype=np.float32)
    print(precisions)
    precisionresult=np.mean(precisions)
    print(precisionresult)
    f = os.file(os.path.join(__PATH2__,'euclideanprecisions'+num+'_'+str(div)+'.txt'),"a+")
